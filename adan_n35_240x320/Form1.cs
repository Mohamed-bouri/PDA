...
namespace adan
{
    public partial class Form1 : Form
    {
        ...
        private const int FORM_W = 240;
        private const int FORM_H = 320;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Color statusColor = _muted ? Color.Red : Color.Green;

            using (Font timeFont = new Font("Tahoma", 10, FontStyle.Bold))
            using (Font statusFont = new Font("Tahoma", 8, FontStyle.Regular))
            using (Font dateFont = new Font("Tahoma", 8, FontStyle.Regular))
            ...
            using (StringFormat sf = new StringFormat())
            {
                ...
                e.Graphics.DrawString(_dateStr, dateFont, blackBrush, new RectangleF(0, 35, FORM_W, 20), sf);

                // This text will turn RED when muted
                e.Graphics.DrawString(_nextStr, statusFont, alertBrush, new RectangleF(0, 55, FORM_W, 20), sf);

                for (int i = 0; i < 6; i++)
                {
                    string timeStr = _prayers[i].Time.ToString("HH:mm");
                    e.Graphics.DrawString(timeStr, timeFont, blackBrush, 5, 83 + (i * 28));
                }

                // Visual Icon/Square in the top left corner
                if (_muted)
                {
                    if (_muteImage != null)
                        e.Graphics.DrawImage(_muteImage, 4, 6);
                    else
                        e.Graphics.FillRectangle(alertBrush, 10, 65, 12, 12);
                }
            }
        }
        ...
        public void TestAlarmSound()
        {
            ...
            if (File.Exists(preFajrPath))
                PlaySoundCE(preFajrPath, IntPtr.Zero, SND_ASYNC | SND_FILENAME);
