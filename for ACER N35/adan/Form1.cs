...
namespace adan
{
    public partial class Form1 : Form
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Color statusColor = _muted ? Color.Red : Color.Black;

            using (Font timeFont = new Font("Tahoma", 10, FontStyle.Bold))
            using (Font statusFont = new Font("Tahoma", 8, FontStyle.Bold))
            ...
            using (StringFormat sf = new StringFormat())
            {
                ...
                e.Graphics.DrawString(_dateStr, dateFont, blackBrush, new RectangleF(0, 48, FORM_W, 20), sf);

                // This text will turn RED when muted
                e.Graphics.DrawString(_nextStr, statusFont, alertBrush, new RectangleF(0, 65, FORM_W, 20), sf);

                for (int i = 0; i < 6; i++)
                {
                    string timeStr = _prayers[i].Time.ToString("HH:mm");
                    e.Graphics.DrawString(timeStr, timeFont, blackBrush, 15, 82 + (i * 28));
                }

                // Visual Icon/Square in the top left corner (X:10, Y:65)
                if (_muted)
                {
                    if (_muteImage != null)
